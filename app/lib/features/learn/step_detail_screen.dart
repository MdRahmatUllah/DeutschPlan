...
/// What each inner tab holds. Words (#114), Grammar (#115) and Quiz (#116)
/// arrive in their own issues and Exams with M4; until then a tab says which
/// it is, so a deep link to `?tab=exams` visibly lands on Exams.
class StepTabBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tokens = context.tokens;
    final l10n = AppLocalizations.of(context);
    // ponytail: placeholders until #114, #115, #116 and M4's exam hub.
    return Center(
      child: DpText(
        switch (tab) {
          StepTab.words => l10n.stepTabWords,
