...
ClozeGap? clozeGap(String sentence, String german, {String? pos}) {
  ...
  // Capitalised past the first word: a noun ("sich Sorgen machen" too).
  bool noun(int i) =>
      (i > 0 || reflexive) && words[i] != words[i].toLowerCase();
  ...
  for (final i in order) {
    if (keys[i].length < minKey) continue;
    final infinitive =
        !noun(i) && keys[i].length > minKey + 1 && keys[i].endsWith('n');
    final hit = _find(tokens, keys[i], verb: infinitive);
...
ClozeGap? _find(List<_Token> tokens, String key, {required bool verb}) {
