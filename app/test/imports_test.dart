...
void main() {
  test('no file under lib/ imports the in-SDK Material or Cupertino libraries', () {
    ...
    for (final file in Directory('lib').listSync(recursive: true).whereType<File>()) {
      ...
      for (final entry in banned.entries) {
        if (source.contains("'${entry.key}'") || source.contains('"${entry.key}"')) {
          offenders.add('${file.path}: imports ${entry.key} — use ${entry.value}');
        }
      }
    }
