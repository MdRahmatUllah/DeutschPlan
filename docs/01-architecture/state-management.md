...
| `modelCard(id)` | autoDispose Stream | M4 | One model's card (#155): its manifest entry, what is on the phone (`ModelRepository.stateOf`), its download as `modelDownloads.watch` moves it, and the bytes a new download lacks. The files are read again at each word from the manager. `phoneSpace` (autoDispose Future) is the storage card's free and total. The download manager behind them is kept alive; the cards need not be. |
