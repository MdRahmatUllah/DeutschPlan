...
- Text scaling to 200 % on every screen, and no word broken mid-syllable to fit it (#165):
  ...
  - Typing at large text, the keyboard leaves a few lines, and what is asked stays in them with the field (#554). Past 130 % with the keyboard up (`DpScript.largeTyping`), L8's header and L12's band keep only their colour behind the status bar, and L8's answer caption and L12's number row go. L12's buttons scroll under the field (Writing's count line takes their place, #529), and L8's *Check* and the umlaut row stay pinned. The #165 audit has no keyboard, so each screen has a test with one.
