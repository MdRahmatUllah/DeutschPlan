...
- Text scaling to 200 % on every screen, and no word broken mid-syllable to fit it (#165):
  - Above 100 % a long German compound, or a Bangla word, in a narrow place may break at a syllable (`DpScript.allowBreaks`, soft hyphens). Only above 100 %: a soft hyphen splits the font's kerning, so 100 % stays as drawn. A line that ends at a syllable shows its "-" (#419), as print does. Flutter breaks at a soft hyphen but draws nothing, so the headword and single-script `DpText` break their own lines greedily, keeping room for the "-", and give their paragraph that text (`_Hyphenated`). A screen reader still hears the word whole, in its own voice. A caption breaks only inside a word too long for a line: T2's and W1's forms break at syllables only in a compound of 15 letters or more, and a shorter word wraps whole.
