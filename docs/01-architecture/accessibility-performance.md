...
- Targets ≥ 48 dp / 44 pt (#478). A control drawn smaller keeps its drawn size and takes `AdaptiveTapTarget`, which grows its hit area and its screen-reader node to 48 dp (44 pt under iOS chrome) without growing its layout. It is the control's own semantics node, so the control's own `Semantics` isn't a container. `goldenTest` checks every case. Two exceptions:
  - iOS's sliding segmented control draws its segments 28 pt, as UIKit's control is short of 44 too. Growing it is an owner question.
