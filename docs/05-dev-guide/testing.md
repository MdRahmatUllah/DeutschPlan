...
- `goldenTest` also runs each case once more, as `'<name> · labels'` (phone, light, after its `act`). Every control a screen reader can press must have a name (`labeledTapTargetGuideline`) and be big enough to press (`androidTapTargetGuideline`, 48 dp; under iOS chrome 44 pt, less the sliding segmented control's segments, #478), and under Material chrome every icon-only control must have a tooltip (`expectIconButtonsTipped`, #162).
