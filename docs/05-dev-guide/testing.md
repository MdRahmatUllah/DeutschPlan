...
- `goldenTest` also runs each case once more, as `'<name> · labels'` (phone, light, after its `act`). Every control a screen reader can press must have a name (`labeledTapTargetGuideline`) and be big enough to press (48 dp; under iOS chrome 44 pt, less a sliding segmented control's segments: selectable buttons in an exclusive group, not radios; a node whose semantics identifier starts `dense:` is skipped as too dense to grow, Me's step badges; #478), and under Material chrome every icon-only control must have a tooltip (`expectIconButtonsTipped`, #162).
