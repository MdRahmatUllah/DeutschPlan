...
- **Writing with the keyboard up** (#529): the task and the text share a few lines on a phone, so the live count line ("N words · min 100" and the connectors found) takes the *Previous / Submit text* row's place above the keyboard, where it stays in view and gives the text that room. The buttons come back when the keyboard goes: a tap anywhere outside the text (the task, the band, the chips) closes it, as a multiline field has no *Done* key and iOS no back gesture that would; the umlaut keys, gaps included, count as the field. The other typed questions keep their buttons, except past 130 % text (#554), or at any size when the keyboard leaves less than 360 dp under the status bar (`shortRoom`, #571: on a 360 × 640 phone Gboard leaves 336, and at 100 % the field ended under the pinned buttons): there the field alone filled the few lines above the keyboard and the prompt scrolled away, so while the keyboard is up the band keeps only its colour behind the status bar, the question's number and flag row goes, and *Previous / Next* scroll under the field. The clock doesn't go: a timed paper always shows its time left, Coral in the last 2 minutes, so the band's clock chip moves to the bar above the keyboard, beside the ä ö ü ß keys, or alone there for an answer typed in English (#560), on a thinner bar than the keys' (#568). What is asked is one type role smaller while typing past 130 % (the word display → headline, a prompt headline → title, a gap's sentence title → body large), and where it still doesn't fit (at 200 %, a gap question on SQA's 411 × 731 phone too, and on a 360 × 640 phone the vocabulary word as well) the list scrolls, the field first: it shows whole before the keyboard comes up, and scrolling back up shows it while the field keeps the keyboard (#573). Writing's field is 120 dp rather than 150 while typing past 130 %, its text scrolling inside as ever: in Bangla at 200 % the pinned count line leaves the list 141 dp (#590). The prompt shows above the field and the umlaut row, and all of it comes back with the keyboard's going, as on L8 (`quiz.md`).
