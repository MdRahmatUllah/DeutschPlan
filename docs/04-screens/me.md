...
- FR-M1-02 Heat-map: `daily_stats` per day, 5 shade steps (0 / 1–9 / 10–19 / 20–39 / ≥ 40 cards). A day's count is every item practised (`new_done + reviews_done + grammar_done + sentences_done`), the activity the streak counts, so a day inside the streak is never empty. Twelve columns, a week each, Monday to Sunday top to bottom, the last the current week; days still to come are drawn empty. Shades are Lagoon at 15 / 35 / 65 / 100 % over Oat (the artboard draws the top three). The header's "days studied" is every day with a non-zero count.
