...
- **Everything** (FR-M7-02):
  ...
  - The field opens focused, so the keyboard is up. Above it, the title, the message and the field scroll, and the actions stay under them. At 200 % or in Bangla they are taller than the room the keyboard leaves, and the actions used to cover the message and the field (#432). When the keyboard opens, the field comes above it with nothing typed yet: the dialog takes the keyboard's room at once, not over Material's 100 ms, so at 200 % on a 360 × 640 phone the field no longer stays under the keyboard until typing (#586). The Cupertino actions draw Bangla in the app's Bengali font.
