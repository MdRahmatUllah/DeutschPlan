...
- R1's no-results page opens R2 with the German filled in (`?german=`). The umlaut row shows while the German field is typed in, and neither German field is autocorrected. The form's list runs under the status bar (its header bleeds to the top), so a focused field keeps the status bar's height clear above it as well as its 20 dp: in Bangla at 200 % with the keyboard up the German field had stopped 3 dp under the status bar (#590).
...
- **T2** serves it as a course word's card: the headword with its article, a *My word* chip where the step would be, the meaning (in the English slot, so every meaning language shows it) and the learner's example, if any. There's no tip, and it keeps the plain card even when the rule says cloze: the cloze's footnote points to W1's card toggle, which R2 doesn't have. *Word details* in the overflow opens it here, in R2. It is rated and scheduled with the same FSRS, and planned again when due, like any word.
- **Quizzes** (FR-R2-04): with *My words in quizzes* on in M3 (`quiz_custom_words`, off by default), all-learned quizzes also ask the learner's words once they have been reviewed. Their meaning is the English one, and the distractors come from the step of the course word they matched, else the step being studied. L9's *Retry mistakes* asks them again, since its compare set names them, and L9's mistake rows show them as saved. Step and category quizzes never ask them, and neither does an exam: its pool is the step's course words.
- **Delete** takes the schedule with it: `word_state` and the open plan rows. Its reviews stay in `review_log` and the done rows, as the days' history. A word deleted while its card is still open in T2 can have its state written again by a rating, but the plan never picks a word whose `custom_words` row is gone. The confirm says "It goes from My words and from revision."
