...
**Custom quiz sheet** (bottom sheet): Direction DE → EN · DE → বাংলা · EN → DE · Articles · Listening · Mixed; Length 10 · 20 · 30; Source this step's learned words · all learned · {category}, the step's category with the most learned words (from any step, as the quiz draws them), and on a tie the step's biggest. That chip stays closed until 10 of its words are learned, as L2's quizzes do, and a caption says so: "{category} opens once 10 of its words are learned · 3 so far" (#337); Timer switch ("Off · 15 s per question when on"); *Start quiz · 20 questions*.
