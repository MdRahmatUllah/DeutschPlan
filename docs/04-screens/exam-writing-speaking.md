...
- The recording is the row's `given` (the path); deleting it removes the file and clears `given`, which zeros the section as #84 grades it (FR-L12S-04). Each tick is written to `self_rubric_json` as it is ticked, and the ticks come back on resume (FR-L12S-03). Until there is a recording the ticks are dimmed and can't be ticked, as L13 shows them for a task without one: #84 counts them only with a recording (#396).
