...
- FR-T5-03 Tapping a token opens a mini sheet: course meaning if the token (stem) matches a word; otherwise translation if `mt_enabled`, else Duden link.
  - How a token matches a word (#324): its search key itself; else a form the course gives in `words.forms` ("ist" is *sein*, "gibt" *geben*, "Häuser" *Haus*; a perfect's auxiliary, a superlative's "am" and a separable verb's particle don't count, nor do a phrase's forms); else a word it is plus an inflection ending ("Wohnungen", "leichter"). A compound is not its first part: "Hausfrau" is not *Haus*. The forms are indexed in memory on the first tap.
