...
- **The notification's texts** are set when files are queued, in the UI language of that moment: queued off Wi-Fi under *Wi-Fi only*, "Waiting for Wi-Fi · 0 of 7 files · Downloads on Wi-Fi, in the background"; otherwise "Downloading models · 0 of 7 files". The platform keeps a task's texts for the whole download, so files queued off Wi-Fi keep the waiting title after Wi-Fi arrives (the count moves), a Wi-Fi drop mid-download still reads as downloading, and a task the system reschedules keeps the language it was queued in (#428).
