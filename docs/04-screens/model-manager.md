...
- **At launch** `attach()` sets one progress notification for every model file ("Downloading models · 2 of 5 files", with a progress bar on Android), listens to the downloader, and calls its `start()`. Tasks the system or the learner killed are scheduled again, and a download that finished while the app was away reports now.
...
- **Activation:** only when every file of the variant is complete are the checksums read (*Verifying*). The variant is put in place only if every one passes (`ModelRepository.activate`); otherwise the phase is *Failed*, and nothing is in place.
- ***Pause* / *Resume*** act on the model's own files. ***Retry*** throws the failed staging away (a corrupt file resumed would never verify) and queues the files again.
