...
4. Contextual card (max one): exams unlocked · voice download · backlog pause offer · content update · step complete (auto-advance off).
...
- *Course finished*: revision-only mode with a completion card.
...
- FR-T1-07 The backlog pause offer appears when backlog > 3 × daily_new (BR-PLAN-07); the content-update card while `content_updates.seen = 0` (BR-CONTENT-03).
