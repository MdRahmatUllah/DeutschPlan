...
3. Write the diff to `user.db.content_updates` (version, added, removed, changed_json = the `added`, `removed`, `changed` and `meaning` lists, seen=0). Today shows the update card while `seen = 0`; its counts are `added`, `removed` and `changed`.
