...
Adding a version is four steps: bump `schemaVersion`, change `user_schema.drift`, run `make schema-dump`, run `make gen`. `onUpgrade` is drift's generated `stepByStep()`, which raises for a version it has no step for rather than opening a learner's file against a schema it was never migrated to. `test/db/migration_test.dart` then opens every fixture and migrates it forward with no edit of its own, and also checks the live DDL still matches the fixture for its own version — the mistake that actually happens is editing the schema and forgetting to re-dump.
