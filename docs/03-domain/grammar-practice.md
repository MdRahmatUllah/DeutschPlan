...
- **Pick the form's sentence.** It is never the gap fill's, whose feedback would answer it. It uses the example's next sentence when there is one, else a course example (`word_examples`) with a word of the gap's sentence, the best-scored first. Only topics with no example, practising a one-sentence English rule, have no other sentence, and they blank another word of it.
- **Its word** has two *real* wrong forms, where the sentence has one. Real means its classes' other members (the inflecting ones above, plus the prepositions, *am/im/um*, the da-/wo-compounds and the question words), the forms the rule names, or an ending or umlaut changed into a form the course uses. A made-up form ("warteen", "Montager") is only a last resort, for a word with nothing better, so it shows only in the English rules of topics with no example. Across the shipped course 2 % of wrong forms are outside the course, against 71 % before #330. `grammar_item_generator_test.dart` holds that at 5 %.
