...
- Directions: `deEn`, `deBn`, `enDe`, `articles`, `listening`, `forms`, `mixed` (round-robin of the above that apply), and `compare` (W2's *Quiz these*, never in a mix).
...
- **What each item asks.** `prompt` is shown and `expected` is checked by `answer_check`:
  ...
  | compare | a set's sentence with the member gapped (`___`), its English as the hint | the member, as the set writes it | the tile, exactly |
...
- **Mixed** gives item *i* the first direction, starting at position *i* of the rotation (deEn, deBn, enDe, articles, listening, forms), that applies to its word. It leaves out the meaning language the learner didn't choose (#339): deBn for `meaning_language = en`, deEn for `bn`; `both` keeps both. A Bangla distractor never reads as the answer: the same meaning once a bracketed qualifier is dropped from one of the two is a synonym ("সাজানো" beside "সাজানো (ঘর)"), though two qualified ones ("তোমাকে (accusative)" / "(dative)") stay the test.
...
- **Compare (#142).** `compare` with source `compareSet` reads `sourceRef` as a set word's uid (`QuizStore.compareSet`), not a list of uids, and builds its items with `compareQuizItems` (`domain/compare_set.dart`) rather than word by word. Every example sentence of the set's members — each resolved member's own, then the set's each is about — is asked once, the member gapped where `clozeGap` finds it, but only a sentence that names a single member (another member's name would give the answer away); items are drawn in a seeded shuffle, up to `length`. The tiles are the members' headwords (the answer and at most three others, seeded), shown for every compare item (`QuizItem.tiles`), and a tile is right only when it is the answer. **Rating:** a compare answer is rated into FSRS only for a word already learning or done — the member's course word, or the set word for a member with none; a To-do word is not started and a suspended one not moved, since a set's members are not the learned words the other sources draw (BR-QUIZ-01). The answer is recorded in `quiz_answers` and counts in L9 either way; L9 shows a compare mistake by the member it asked for (`expected`). The runner titles it "Compare" and asks "Which word fits?". *Retry mistakes* of a compare quiz is the same set again, as many items as it had mistakes.
