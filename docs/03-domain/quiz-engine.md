...
- **Compare (#142).** `compare` with source `compareSet` reads `sourceRef` as a set word's uid (`QuizStore.compareSet`), not a list of uids, and builds its items with `compareQuizItems` (`domain/compare_set.dart`) rather than word by word. Every example sentence of the set's members — each resolved member's own, then the set's that name a member — is asked once, the member gapped where `clozeGap` finds it; items are drawn in a seeded shuffle, up to `length`. The tiles are the members' headwords (the answer and at most three others, seeded), shown for every compare item (`QuizItem.tiles`), and a tile is right only when it is the answer. Each answer is rated into FSRS for the member's course word, or the set word for a member the course has none for. The runner titles it "Compare" and asks "Which word fits?". *Retry mistakes* of a compare quiz is the same set again, as many items as it had mistakes.
