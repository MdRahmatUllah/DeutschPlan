...
- `services/widget_snapshot.dart` builds it from T1's view of today: `{date, step, remaining, total, minutes, done, wordOfDay{uid, article, german, meaning}, tomorrow{revise, newWords, minutes}, copy{…}}`. `remaining` and `total` are the ring's (FR-T1-02) and `minutes` its estimate. `tomorrow` is set once the day is done, for the Lime-check state, and is null before. `date` lets a widget tell yesterday's snapshot from today's.
- It is stored as the JSON string under `widget_snapshot` in `home_widget`'s shared store: SharedPreferences on Android, and the App Group `group.app.deutschplan`'s UserDefaults on iOS (release.md). #160 and #161 read it there; on Android the write is followed by `updateWidget`, which redraws the widget at once.
...
Details the Android widget settles (#160):
- A Glance widget (`android/…/widget/DeutschPlanWidget.kt`) behind `home_widget`'s Glance receiver, which hands it the snapshot. `SizeMode.Responsive` draws two layouts: small below 250 dp wide, medium from there on. It places at 2 × 2 and stretches to 4 × 2 or more, uncapped, because cells differ by launcher. The small layout pins the step and the name to the top, and the ring to the bottom, as the artboard does.
- Its words come in the snapshot as `copy`, already in the learner's UI language: `app`, `left` ("8 left"), `leftStep` ("8 left · A2.1"), `progress` ("12/20"), `minutes`, `wordOfDay` (German in every language), `pronounce`, `done`, and `tomorrow` ("Tomorrow · 12 revisions · 7 new", once the day is done). The native side can't read the ARB files, and a snapshot written in the old language is rewritten when the language changes. Before the app has written one, the widget says so. That string, `widget_empty`, and the picker's `widget_description` are the only strings in `res/values` (English, with `values-bn` for Bangla).
- Its colours are the tokens, written out in `res/values/colors.xml` and `values-night/colors.xml`: light, and dark when the phone is dark. The glass theme draws as its opaque fallback (FR-X1-04). `test/features/widget_native_test.dart` stops the two copies drifting, and checks that the redraw names the receiver the manifest declares. Glance has no determinate ring, so the arc is a white bitmap tinted Lagoon from a resource; the track and the done state's Lime check are drawables. Every colour is a resource, so a switch between light and dark shows at once.
- Done: the Lime check and "Done for today", then tomorrow's line. With no word of the day, the medium size shows the minutes (or, done, tomorrow) in its place. The artboard doesn't draw that case.
- Taps open `MainActivity` with the link as its data (FR-X1-02): the widget opens `deutschplan://today`, the word opens `deutschplan://word/<uid>`, and *Pronounce* opens the same link with `?speak=1`. `deep_links.dart` routes them.
- Tests: the snapshot's `copy` in English and Bangla, and the colours. There are no Flutter goldens for a native widget, so it was checked on the emulator at both sizes, light and dark, before and after the day was done. The picker's preview is the app icon: a `previewLayout` is left for when the launcher's picker matters.
