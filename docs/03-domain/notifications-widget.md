...
- Written at midnight (`plan_pregenerate`), hourly (`widget_refresh`), and in the app whenever the snapshot changes (`followWidget`, from launch). A finished session always changes it, which is FR-X1-01's "after every session". An unchanged snapshot isn't written again, and a store that can't be written costs one refresh, never the task.
