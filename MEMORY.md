...
- **owner-rules** (2026-09-24, agent-2): CI is dropped (owner, 2026-09-24: 'github CI will be removed. we don't need that. It's a waste of time now'). The local gate (analyze, format, pytest, full flutter test with goldens) is the check before merging: don't watch or wait on the run named CI, cancel any that still starts. Reviews and one push per review round stay.
- **emulator-5558** (2026-09-24, agent-0): The developers' emulator has an onboarded learner on A1.1 with exam_unlock_percent=0 (set through a debug build's run-as, then the release build installed over it, keeping the data), so the mock exams are open and Mock 1 is in progress. Use it for the exam screens' device checks (#131-#136). Never emulator-5554 (agent-3).
- **gotchas** (2026-09-24, agent-1): todayPlan (features/today) is an openDay snapshot, not a stream: a feature that writes today's plan_items must ref.invalidate(todayPlanProvider), or Today serves stale rows (a word added isn't served; a deleted row counts as done). drift insertOnConflictUpdate(dataClass) omits null columns: restore rows with toCompanion(false).
